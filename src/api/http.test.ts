...
describe('http api', () => {
  ...
  it('reads the formatted total the live backend sends', async () => {
    fakeFetch([{ body: { ok: true, orders: [{ orderNumber: '10043', placedAt: '2026-09-20T12:00:00Z', total: '$1,027.95', stage: 'In review', items: [{ title: 'Poster', quantity: 1 }] }] } }]);
    const [order] = await api().getOrders(DEVICE);
    expect(order.totalCents).toBe(102795);
  });
